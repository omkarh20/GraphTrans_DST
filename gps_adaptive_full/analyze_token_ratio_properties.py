# analyze_token_ratio_properties.py
"""
...
Changes over gps_adaptive version:
  - Added 'avg_head_gate' to correlation analysis
  - Added 'avg_head_gate' to heatmap columns
  - Added 'avg_head_gate' to summary statistics
  - Extended multi-panel plot to 4x3 grid to include head gate panels
"""
...
# Configuration
csv_path = r"exps\DD_adaptive_full_cv10-04_03-21_06_08\fold_1\graph_stats.csv"
...
if 'avg_head_gate' in df_filtered.columns:
    print(f"Head gate range:    [{df_filtered['avg_head_gate'].min():.4f}, {df_filtered['avg_head_gate'].max():.4f}]")
print()

# ── Correlation properties dict ───────────────────────────────────────────────
# Added avg_head_gate as a new property to correlate against token ratio
properties = {
    ...
    'avg_head_gate':   'Average Head Gate',          # ← NEW
}
...
# ── Multi-panel plot (4×3 grid to accommodate head gate panels) ───────────────
fig, axes = plt.subplots(4, 3, figsize=(17, 18))
fig.suptitle('Token Ratio & Head Gate Dependence on Graph Properties',
             fontsize=16, fontweight='bold')

plot_props = [
    # Row 1-2: original token ratio panels
    ...
    (None,              'Distribution: Token Ratios',  None),   # histogram
    # Row 3: NEW head gate panels
    ('avg_head_gate',   'Average Head Gate',      'royalblue'),
    (None,              'Distribution: Head Gates',    None),   # histogram
    (None,              'Head Gate vs Layer Gate',     None),   # scatter
]

for idx, (prop_key, prop_name, color) in enumerate(plot_props):
    ax = axes.flat[idx]

    # ── Distribution: Token Ratios ────────────────────────────────────────
    if prop_key is None and 'Token' in prop_name:
        ax.hist(df_filtered['avg_token_ratio'], bins=30, alpha=0.7,
    ...
    # ── Distribution: Head Gates (NEW) ────────────────────────────────────
    elif prop_key is None and 'Head Gates' in prop_name:
        if 'avg_head_gate' in df_filtered.columns:
            ax.hist(df_filtered['avg_head_gate'], bins=30, alpha=0.7,
                    color='royalblue', edgecolor='black')
            ax.axvline(df_filtered['avg_head_gate'].mean(), color='red',
                       linestyle='--', linewidth=2, label='Mean')
            ax.axvline(df_filtered['avg_head_gate'].median(), color='orange',
                       linestyle='--', linewidth=2, label='Median')
            ax.set_xlabel('Avg Head Gate', fontsize=11)
            ax.set_ylabel('Frequency', fontsize=11)
            ax.set_title(prop_name, fontsize=12, fontweight='bold')
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3, axis='y')
    ...
    # ── Head Gate vs Layer Gate scatter (NEW) ────────────────────────────
    elif prop_key is None and 'Head Gate vs' in prop_name:
        if 'avg_head_gate' in df_filtered.columns and 'avg_layer_gate' in df_filtered.columns:
            ax.scatter(df_filtered['avg_layer_gate'], df_filtered['avg_head_gate'],
                       alpha=0.6, s=40, color='darkorange')
            rho, pval = spearmanr(df_filtered['avg_layer_gate'],
                                   df_filtered['avg_head_gate'])
            ax.text(0.05, 0.95, f'ρ={rho:+.3f}',
                    transform=ax.transAxes, verticalalignment='top',
                    bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.7),
                    fontsize=9)
            ax.set_xlabel('Avg Layer Gate', fontsize=11)
            ax.set_ylabel('Avg Head Gate', fontsize=11)
            ax.set_title(prop_name, fontsize=12, fontweight='bold')
    ...
    # ── Standard scatter vs token ratio ──────────────────────────────────
    else:
        ...
        ax.scatter(df_filtered[prop_key], df_filtered['avg_token_ratio'],
                   alpha=0.6, s=40, color=color)

        valid_mask = df_filtered[prop_key].notna() & df_filtered['avg_token_ratio'].notna()
        if valid_mask.sum() > 1:
            z = np.polyfit(df_filtered[prop_key][valid_mask],
                           df_filtered['avg_token_ratio'][valid_mask], 1)
            p = np.poly1d(z)
...
# ── Correlation heatmap (includes avg_head_gate) ──────────────────────────────
fig, ax = plt.subplots(figsize=(11, 10))

heatmap_cols = [
    'num_nodes', 'log_num_nodes', 'num_edges', 'log_num_edges',
    'density', 'avg_degree', 'degree_variance',
    'avg_layer_gate', 'avg_head_gate',          # ← avg_head_gate added
    'avg_token_ratio',
]
...
sns.heatmap(heatmap_data, annot=True, fmt='.2f', cmap='RdBu_r', center=0,
            ...
            annot_kws={'size': 9})
ax.set_title('Spearman Correlation Matrix (All Graph Properties)',
...
if 'avg_layer_gate' in df_filtered.columns:
    print(f"Layer gate     — Mean: {df_filtered['avg_layer_gate'].mean():.4f}, Std: {df_filtered['avg_layer_gate'].std():.4f}")
if 'avg_head_gate' in df_filtered.columns:                                   # ← NEW
    print(f"Head gate      — Mean: {df_filtered['avg_head_gate'].mean():.4f}, Std: {df_filtered['avg_head_gate'].std():.4f}")
...
for prop_key, rho in sorted_corrs[:6]:                                       # top 6 now
...
# - Layer Gate (moderate positive): Active layers keep more tokens; inactive layers skip

# Head Gate Analysis (NEW):
# - Head gate vs num_nodes: if negative → larger graphs suppress more heads
# - Head gate vs layer gate: if positive → active layers also retain more heads
# - Head gate vs density: if negative → denser graphs may need fewer heads
# - Low head gate variance → BudgetNet converged to uniform head suppression
# - High head gate variance → BudgetNet learned graph-specific head selection

# Implications for Paper:
# - Token ratio driven by graph size (O(k²) attention cost)
# - Head gating adds a third dimension of adaptive sparsification
# - Full system: Token + Layer + Head pruning covers all axes of computation
# """)
# print("=" * 80 + "\n")
