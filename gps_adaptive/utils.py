...
def results_to_file(args, test_acc, val_acc):
    ...
    headerList = ["Method", "lambda_compute",
                  "::::::::",
                  "test_acc", "val_acc"]
    ...
    with open(filename, "a+") as f:
        ...
        line = "{}, {}, :::::::::, {:.4f}, {:.4f}\n".format(
            args.model_type, args.lambda_compute,
            test_acc, val_acc
        )
        f.write(line)
...
def results_cv_to_file(args, fold_results):
    """
    Log cross-validation aggregated results.

    Parameters
    ----------
    args : argparse.Namespace
    fold_results : list of dict
        Each dict has keys: 'fold', 'val_acc', 'test_acc', 'avg_token_ratio'
    """
    if not os.path.exists('./results/{}'.format(args.dataset)):
        os.makedirs('./results/{}'.format(args.dataset))
    ...
    # --- Write per-fold rows ---
    headerList = ["Method", "lambda_compute", "n_folds",
                  "fold", "val_acc", "test_acc", "avg_token_ratio", "flop_reduction"]
    ...
    with open(summary_file, "w", newline='') as f:
        headers = ["Method", "lambda_compute", "n_folds",
